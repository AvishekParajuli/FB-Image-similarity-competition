...
def embedding_model1( base_cnn, drop=0.3, embeddingdim =256,finetune=False):
...
def embedding_model2(inputhead, drop =0.3, embeddingdim = 160):
    pad = "same"
    nf =128
    x = Conv2D(filters=nf, kernel_size=7, strides=3,
     activation="relu",padding= pad, name ="em2_conv1")(inputhead)
    #print("x shape:", x.shape)
    x = MaxPool2D(pool_size = 4, strides = 4,padding=pad)(x)
    #print("x shape:", x.shape)
    x = Conv2D(filters=nf, kernel_size=5, strides=2,
     activation="relu",padding= pad, name ="em2_conv2")(x)
    #print("x shape:", x.shape)
    x = MaxPool2D(pool_size = 4, strides = 4,padding=pad)(x)
    #print("x shape:", x.shape)
    x = Flatten()(x)
...
def embedding_model3(inputhead, drop =0.3, embeddingdim = 160):
    pad = "same"
    nf = 128
    x = Conv2D(filters=nf, kernel_size=9, strides=4,
     activation="relu",padding= pad, name ="em3_conv1")(inputhead)
    #print("x shape:", x.shape)
    x = MaxPool2D(pool_size = 4, strides = 4,padding=pad)(x)
    #print("x shape:", x.shape)
    x = Conv2D(filters=nf, kernel_size=4, strides=2,
     activation="relu",padding= pad, name ="em3_conv2")(x)
    #print("x shape:", x.shape)
    x = MaxPool2D(pool_size = 3, strides = 3,padding=pad)(x)
    #print("x shape:", x.shape)
    x = Flatten()(x)
    print("x shape:", x.shape)
    #x = GlobalAveragePooling2D()(x)
    ...
    #mdl = Model(inputs=input_head2, outputs=output, name="Embedding2")
    return output
def embedding_model_3(inputhead,drop =0.3, embeddingdim = 160):
...
def embedding_model(drop=0.3, embeddingdim=512, freezeFirstHead=False, firstHeadWt=""):
