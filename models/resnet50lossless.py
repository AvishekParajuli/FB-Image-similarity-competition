...
def embedding_model():
  ...
  output = keras.layers.Dense(256,activation='sigmoid')(dense1)
  #output = Lambda(my_norm)(output)
  #we are forcing output to be 0 or 1 to use with lossless func
...
def complete_model(base_model):
    ...
    loss = Lambda(triplet_loss_lossless)([A, P, N])
    model = Model(inputs=[input_1, input_2, input_3], outputs=loss)
    model.compile(loss=identity_loss, optimizer=Adam(LR))
    return model

def get_model_name():
    return "resnet50lossless"
def preprocess(x):
